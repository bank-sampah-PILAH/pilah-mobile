...
/// Commands:
///   list
///   add <name> [--with-storage] [--state bloc|cubit|riverpod]
...
void main(List<String> args) async {
  ...
  switch (args[0]) {
    case 'list':
      await _cmdList();
    case 'add':
      if (args.length < 2) _die('Usage: add <name> [--with-storage] [--state bloc|cubit|riverpod]');
      ...
      final stateOverride = stateIdx != -1 && stateIdx + 1 < args.length
          ? args[stateIdx + 1]
          : null;
      await _cmdAdd(args[1], withStorage: withStorage, stateOverride: stateOverride);
    case 'disable':
...
Future<void> _cmdAdd(
  ...
  print('  → Register the route in lib/core/router/app_router_config.dart');
}
